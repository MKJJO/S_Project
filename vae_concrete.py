...
class VAE():
    ...
    def _set_model(self):
        ...
        Q_0 = Conv2D(input_shape[1], kernel_size = (1, 2), padding='same', activation='relu')
        Q_1 = Conv2D(self.filters[0], kernel_size = (1, 2), padding='same', activation='relu')
        Q_2 = Conv2D(self.filters[0], kernel_size = (1, 2), padding='same', activation='relu')
        Q_3 = Conv2D(self.filters[0], kernel_size = (1, 2), padding='same', activation='relu')
        print("Setting up model...")
        Q_4 = Flatten()
        ...
        # distribution later
        out_shape = (self.input_shape[0] / 2, self.input_shape[1] / 2, self.filters[1])
        G_0 = Dense(self.hidden_dim, activation='relu')
        G_1 = Dense(np.prod(out_shape), activation='relu')
        G_2 = Reshape(out_shape)
        G_3 = Conv2DTranspose(self.filters[1], (1, 3), padding='same',
                              strides=(1, 1), activation='relu')
        G_4 = Conv2DTranspose(self.filters[1], (1, 3), padding='same',
                              strides=(1, 1), activation='relu')
        G_5 = Conv2DTranspose(self.filters[0], (1, 2), padding='valid',
                              strides=(1, 2), activation='relu')
        G_6 = Conv2D(self.input_shape[1], (1, 2), padding='same',
